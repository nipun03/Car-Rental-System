...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 17, 2017 at 01:15 AM
...
-- Procedures
--
CREATE DEFINER=`nipun03`@`%` PROCEDURE `GetRentalAmount`(IN `p_license_number` VARCHAR(50), OUT `p_rental_amount` INT(30), IN `p_start` DATE, IN `p_end` DATE, IN `p_price` INT(30))
begin
...
-- Functions
--
CREATE DEFINER=`nipun03`@`%` FUNCTION `GetPenaltyAmount`(`p_license_number` VARCHAR(50), `p_actualend` DATE, `p_end` DATE, `p_price` INT(30)) RETURNS int(30)
    DETERMINISTIC
begin
declare penaltyamount int(50);
set penaltyamount =datediff(p_actualend,end)*price*0.5;
...
('ABX1234', 'CIVIC', 'HONDA', 'Green', 2014, 40, 4, 'FULL SIZE', 'Not Available', 'Admin1', 2),
...
('ZCX5756', 'GOLF', 'VOLKSWAGAN', 'White', 2015, 28, 2, 'ECONOMY', 'Available', 'Admin2', 2);
...
--
-- Stand-in structure for view `EntireDetails`
--
CREATE TABLE IF NOT EXISTS `EntireDetails` (
`fullname` varchar(101)
,`card_number` bigint(30)
,`username` varchar(50)
,`license_number` varchar(30)
,`car_id` varchar(20)
,`car_type` varchar(100)
,`price_per_day` int(30)
,`reservation_id` int(30)
,`start_date` date
,`end_date` date
,`actual_end_date` date
,`penalty_amount` int(30)
,`rental_amount` int(30)
,`amounttobepaid` bigint(31)
,`pickup_location_id` int(30)
,`drop_location_id` int(30)
);
-- --------------------------------------------------------
...
INSERT INTO `Location` (`location_id`, `email_id`, `phone_number`, `address`) VALUES
(1, 'jondoe@gmail.com', 2025550184, 6),
(2, 'jaria@uncc.edu', 2025550117, 9),
(3, 'mike12@gmail.com', 2025550155, 7),
(4, 'lindap@gmail.com', 2025550171, 11),
(5, 'jamar@gmail.com', 2025550112, 10);
...
(7, 'Morgansar', 3726482748274827, 65, 8);
...
INSERT INTO `Reservation` (`reservation_id`, `start_date`, `end_date`, `actual_end_date`, `penalty_amount`, `rental_amount`, `status`, `license_number`, `car_id`, `pickup_location_id`, `drop_location_id`) VALUES
(1, '2017-10-05', '2017-10-07', '2017-10-09', 65, 130, 'Returned', 'K23-434-2918', 'ABX1234', 1, 2),
(2, '2017-11-12', '2017-11-13', '2017-11-13', 0, 90, 'Returned', 'MD4-456-4832', 'XZC2556', 1, 4),
(3, '2018-01-28', '2018-01-31', NULL, NULL, NULL, 'Cancelled', 'FA7-827-0831', 'ZCX5756', 2, 5),
(4, '2017-10-03', '2017-10-06', '2017-10-04', 0, 90, 'Returned', 'lO9-993-9871', 'CXZ2356', 4, 4),
(5, '2017-11-10', '2017-11-13', '2017-11-15', 55, 165, 'Returned', 'C89-384-1938', 'SDL9356', 3, 2),
(7, '2017-11-11', '2017-11-16', NULL, NULL, NULL, 'Booked', 'K23-434-2918', 'ABX1234', 2, 4),
(8, '2017-11-16', '2017-11-18', '2017-11-17', 0, 65, 'Returned', 'MD4-456-4832', 'HJK1234', 4, 2),
(9, '2017-11-11', '2017-11-14', '2017-11-12', 0, 45, 'Returned', 'FA7-827-0831', 'HNX1890', 2, 5);
...
select price_per_day into price from Car inner join CarType using (car_id) where car_id=new.car_id;
...
select username, card_number into p_username, p_cardno from Account inner join CardDetails using (username) where license_number= new.license_number;
insert into Payment(username,card_number,reservation_id,amount_paid ) values(p_username,p_cardno,new.reservation_id,new.rental_amount+new.penalty_amount);
end if;
...
--
-- Structure for view `EntireDetails`
--
DROP TABLE IF EXISTS `EntireDetails`;

CREATE ALGORITHM=UNDEFINED DEFINER=`nipun03`@`%` SQL SECURITY DEFINER VIEW `EntireDetails` AS select concat_ws(' ',`c`.`first_name`,`c`.`last_name`) AS `fullname`,`cd`.`card_number` AS `card_number`,`cd`.`username` AS `username`,`c`.`license_number` AS `license_number`,`r`.`car_id` AS `car_id`,`cr`.`car_type` AS `car_type`,`ct`.`price_per_day` AS `price_per_day`,`r`.`reservation_id` AS `reservation_id`,`r`.`start_date` AS `start_date`,`r`.`end_date` AS `end_date`,`r`.`actual_end_date` AS `actual_end_date`,`r`.`penalty_amount` AS `penalty_amount`,`r`.`rental_amount` AS `rental_amount`,(`r`.`penalty_amount` + `r`.`rental_amount`) AS `amounttobepaid`,`r`.`pickup_location_id` AS `pickup_location_id`,`r`.`drop_location_id` AS `drop_location_id` from ((((((`Customer` `c` join `Account` `a` on((`c`.`license_number` = `a`.`license_number`))) join `CardDetails` `cd` on((`a`.`username` = `cd`.`username`))) join `Reservation` `r` on((`r`.`license_number` = `c`.`license_number`))) join `Car` `cr` on((`r`.`car_id` = `cr`.`car_id`))) join `CarType` `ct` on((`cr`.`car_type` = `ct`.`car_type`))) join `Location` `l` on((`cr`.`location_id` = `l`.`location_id`)));
...
ALTER TABLE `CardDetails`
  ...
  ADD CONSTRAINT `CardDetails_ibfk_2` FOREIGN KEY (`billing_address`) REFERENCES `Address` (`address_id`),
...
--
ALTER TABLE `Payment`
  ADD CONSTRAINT `Payment_ibfk_1` FOREIGN KEY (`card_number`) REFERENCES `CardDetails` (`card_number`),
  ADD CONSTRAINT `Payment_ibfk_2` FOREIGN KEY (`username`) REFERENCES `CardDetails` (`username`),
